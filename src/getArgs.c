...
void getArgs(FILE *output_ptr, int argc, char **argv)
{
    ...
    if (argc < 2){
        log_out(output_ptr, "ERROR: The number of arguments is insufficient for any kind of operation! \n\n");
        usage_error();
    }
    ...
    // Information about the versions of the program
    for( i = 1; i < argc; i++) {
        if (strcmp("-info",argv[i])==0) {
            ...
            log_out(output_ptr, "\t\tDietschreit, J. C. B.; Diestler, D. J.; Knapp, E. W.,\n");
            log_out(output_ptr, "\t\tChemically Realistic Tetrahedral Lattice Models for Polymer Chains: Application to Polyethylene Oxide.\n");
            log_out(output_ptr, "\t\tJournal. 2015, Volume, Pages\n");
    ...
    // SAW1 on tetrahedral lattice fragmented
    for( i = 1; i < argc; i++) {
    ...
    // SAW1 on tetrahedral lattice doubly fragmented
    for( i = 1; i < argc; i++) {
    ...
    // SAW1 on tetrahedral lattice with bricks
    for( i = 1; i < argc; i++) {
    ...
    // SAW1 on tetrahedral lattice fragmented and with bricks
    for( i = 1; i < argc; i++) {
...
void print_set_options(FILE *output_ptr, int typeofrun, int flength, int fflength, int blength, int numberofbeads, unsigned long long numberofframes, long seed, double bond, int torsion, int intramolecular, double intra_eps1[2], double intra_eps2[2]){
    ...
    log_out(output_ptr, "\n-----------------------------------------------------------------------------------\n");
    log_out(output_ptr, "Start of the computation!\n\n");
