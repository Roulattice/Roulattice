    ...
    try_frag_fb_saw1:
        ...
        new_fbrick_fb_saw1:
            ...
            for (int atom=0; atom<blength_var; ++atom) {
                ...
                if (check_fb_saw1(poly_ptr, position, insert_start_f, atom)==true){
                    printf("found error in check_fb_saw1\n");
                    goto try_frag_fb_saw1;
                }
        ...
        for (int bead=(insert_start_b+1); bead<=(insert_start_f+flength); ++bead){
            ...
            if (check_fsaw1(poly_ptr, bead, insert_start_f)==true){
                goto try_frag_fb_saw1; // start again with this very fragment
            }
        ...
        if (check_whole_fsaw1(poly_ptr, insert_start_f, (insert_start_f+flength))==true){
            printf("found error in check_whole_fsaw1\n");
            goto try_fb_saw1; // start all over again
        }
...
try_end_fb_saw1:
    ...
    new_endbrick_fb_saw1:
        ...
        for (int atom=0; atom<blength_var; ++atom) {
            ...
            // look at this checking procedure
            if (check_fb_saw1(poly_ptr, position, insert_start_f, atom)==true){
                goto try_end_fb_saw1;
            }
    ...
    for (int bead=(insert_start_b+1); bead<(numberofbeads); ++bead){
        ...
        if (check_fsaw1(poly_ptr, bead, insert_start_f)==true){
            goto try_end_fb_saw1; // start again with this very fragment
        }
    ...
    // now the fragment itself is completed, now is the time to check it against the previous chain
    if (check_whole_fsaw1(poly_ptr, insert_start_f, (numberofbeads-1))==true){
        goto try_fb_saw1; // start all over again
    }
