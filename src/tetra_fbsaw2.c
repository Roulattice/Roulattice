    ...
    try_frag_fb_saw2:
        ...
        new_fbrick_fb_saw2:
            ...
            for (int atom=0; atom<blength_var; ++atom) {
                ...
                if (check_fb_saw2(poly_ptr, position, insert_start_f, bricks_ptr[randn][atom][3], saw_ptr, move_ptr, (1-odd), atom)==true){
                    goto try_frag_fb_saw2;
                }
        ...
        for (int bead=(insert_start_b+1); bead<=(insert_start_f+flength); ++bead){
            ...
            if (check_fsaw2(poly_ptr, bead, insert_start_f, lastmove, saw_ptr, move_ptr, odd)==true){
                goto try_frag_fb_saw2; // start again with this very fragment
            }
        ...
        if (check_whole_fsaw2(poly_ptr, insert_start_f, (insert_start_f+flength), move_ptr)==true){
            goto try_fb_saw2; // start all over again
        }
...
try_end_fb_saw2:
    ...
    for (int bead=(insert_start_b+1); bead<(numberofbeads); ++bead){
        ...
        if (check_fsaw2(poly_ptr, bead, insert_start_f, lastmove, saw_ptr, move_ptr, odd)==true){
            goto try_end_fb_saw2; // start again with this very fragment
        }
