...
try_bsaw2:
    ...
    new_brick_bsaw2:
        ...
        for (int atom=0; atom<blength_var; atom++) {
            ...
            if (check_bsaw2(poly_ptr, position, bricks_ptr[randn][atom][3], saw_ptr, move_ptr, (1-odd), insert_start)==true){
                goto try_bsaw2;
            }
...
try_end_bsaw2:
    ...
    for (int bead=(insert_start+1); bead<(numberofbeads); bead++){
        ...
        if (check_fsaw2(poly_ptr, bead, insert_start, lastmove, saw_ptr, move_ptr, odd)==true){
            goto try_end_bsaw2; // start again with this very fragment
        }
    }
    ...
    if (check_whole_fsaw2(poly_ptr, insert_start, (numberofbeads-1), move_ptr)==true){
        goto try_bsaw2; // start all over again
    }
