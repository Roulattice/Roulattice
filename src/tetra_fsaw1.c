    ...
    try_frag_fsaw1:
        ...
        for (int bead=(insert_start_f+2); bead<=(insert_start_f+flength); ++bead){
            ...
            if (check_fsaw1(poly_ptr, bead, insert_start_f)==true){
                goto try_frag_fsaw1; // start again with this very fragment
            }
        }
        ...
        if (check_whole_fsaw1(poly_ptr, insert_start_f, (insert_start_f+flength))==true){
            goto try_fsaw1; // start all over again
        }
...
try_end_fsaw1:
    ...
    for (int bead=(insert_start_f+1); bead<(numberofbeads); ++bead){
        ...
        if (check_fsaw1(poly_ptr, bead, insert_start_f)==true){
            goto try_end_fsaw1; // start again with this very fragment
        }
